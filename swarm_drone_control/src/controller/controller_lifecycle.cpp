...
LifecycleCallbackReturn GamepadController::on_configure(const rclcpp_lifecycle::State &)
{
    ...
    std::string NEIGHBORS_INFO = "/px4_" + std::to_string(sys_id) + "/neighbors_info";
    ...
    this->joy_subscriber_ = this->create_subscription<sensor_msgs::msg::Joy>(
        ...
        { this->joystick_callback(msg); });
    this->vehicle_attitude_subscriber_ = this->create_subscription<px4_msgs::msg::VehicleAttitude>(
        VEHICLE_ATTITUDE, qos, [this](const px4_msgs::msg::VehicleAttitude::SharedPtr msg)
        { this->altitude_callback(msg); });
    this->vehicle_status_subscriber_ = this->create_subscription<px4_msgs::msg::VehicleStatus>(
        VEHICLE_STATUS, qos, [this](const px4_msgs::msg::VehicleStatus::SharedPtr msg)
        { this->vehicle_status_callback(msg); });
    this->neighbors_info_subscription_ = this->create_subscription<custom_interfaces::msg::NeighborsInfo>(
        NEIGHBORS_INFO, qos, [this](const custom_interfaces::msg::NeighborsInfo::SharedPtr msg)
        { this->neighbors_info_callback(msg); });

    this->offboard_setpoint_counter_ = 0;

    this->timer_ = this->create_wall_timer(100ms, [this]()
